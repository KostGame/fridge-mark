...
PR002 добавляет локальное сканирование DataMatrix / QR через ZXing и парсер GS1 Application Identifiers. Локальный парсер извлекает только те поля, которые реально присутствуют в коде: если срок годности не закодирован, его нужно указать вручную. Все пользовательские данные хранятся локально в браузере через IndexedDB. Официальный API Честного знака, backend и Android APK пока не реализованы.

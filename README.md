...
PR001 — это первый прототип без сервера. Все пользовательские данные хранятся локально в браузере через IndexedDB. Официальный API Честного знака, реальный сканер и Android APK пока не реализованы.
...
- реальное сканирование DataMatrix;
- парсер GS1 Application Identifiers;
